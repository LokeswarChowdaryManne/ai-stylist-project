...
# --- Pydantic Models (Added one new model for the tag response) ---
...
class AutoTagResponse(BaseModel): tags: NewClothingItem # Model for our auto-tag response

# (Weather and Cache functions are the same)
WEATHER_API_KEY = "ac058b95d5d3473d9d0ecc0dac09c9ba"
COIMBATORE_LAT = 11.0168; COIMBATORE_LON = 76.9558
...
# --- API Endpoints ---
@app.get("/")
def read_root(): return {"message": "Stylist Backend is running. Go to /docs."}

# NEW ENDPOINT 1: Just for auto-tagging
...
# NEW ENDPOINT 2: Adds the item after user has verified/corrected the tags
...
# (Suggest and Delete endpoints are the same)
@app.get("/suggest/{user_id}", response_model=OutfitResponse)
def suggest_for_user(user_id: int, occasion: str):
    weather = get_current_weather()
    if not weather: raise HTTPException(status_code=503, detail="Weather service unavailable.")
